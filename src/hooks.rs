...
pub enum HookResultStatus { Succeeded, Failed, TimedOut }
...
pub struct HookRunResult { pub scripts: Vec<HookResult> }

impl HookRunResult {
    pub fn failed(&self) -> bool { self.scripts.iter().any(|r| r.status != HookResultStatus::Succeeded) }
}

pub struct HookRunner { timeout: Duration, output_limit: usize }

impl Default for HookRunner {
    fn default() -> Self { Self { timeout: DEFAULT_TIMEOUT, output_limit: DEFAULT_OUTPUT_LIMIT } }
}

impl HookRunner {
    pub fn new(timeout: Duration, output_limit: usize) -> Self { Self { timeout, output_limit } }

    pub fn run(&self, hooks: &HookConfiguration, event: HookEvent, payload: &HookPayload, logger: &DiagLogger) -> HookRunResult {
        let mut input = match serde_json::to_vec(payload) {
            Ok(value) => value,
            Err(error) => { logger.error("hook.serialize", &safe_detail(&error.to_string())); return HookRunResult::default(); }
        };
        input.push(b'\n');
        let mut result = HookRunResult::default();
        for path in hooks.paths_for(event, &payload.working_dir) { result.scripts.push(self.run_one(path, &input, payload, logger)); }
        result
    }

    fn run_one(&self, path: PathBuf, input: &[u8], payload: &HookPayload, logger: &DiagLogger) -> HookResult {
        let started = Instant::now();
        let mut command = Command::new(&path);
        command.current_dir(&payload.working_dir).stdin(Stdio::piped()).stdout(Stdio::piped()).stderr(Stdio::piped())
        ...
        let child = match command.spawn() {
            ...
            Err(error) => {
                ...
                return HookResult { path, status: HookResultStatus::Failed, duration: started.elapsed(), exit_code: None, stdout: String::new(), stderr: String::new(), failure: Some(failure) };
            }
        };
    ...
    fn finish_child(&self, mut child: std::process::Child, path: PathBuf, input: &[u8], started: Instant, logger: &DiagLogger) -> HookResult {
        ...
        let (result_status, failure) = if timed_out {
            (HookResultStatus::TimedOut, Some(format!("timed out after {}ms", self.timeout.as_millis())))
        } else if status.as_ref().map(|s| s.success()).unwrap_or(false) { (HookResultStatus::Succeeded, None) }
        else { (HookResultStatus::Failed, Some(format!("exited with status {}", status.map(|s| s.to_string()).unwrap_or_else(|| "unknown".into())))) };
...
fn read_bounded<R: std::io::Read>(mut reader: R, limit: usize) -> String {
    ...
    while bytes.len() < limit {
        ...
        match reader.read(&mut buffer[..max]) { Ok(0) | Err(_) => break, Ok(count) => bytes.extend_from_slice(&buffer[..count]) }
    }
    String::from_utf8_lossy(&bytes).into_owned()
}

fn safe_detail(value: &str) -> String { value.chars().filter(|c| !c.is_control() || *c == '\n' || *c == '\t').take(4096).collect() }

#[cfg(test)]
mod tests {
    use super::*;
    use crate::config::{HookStatus, HookPayload};
    use std::fs;
    use tempfile::tempdir;

    fn payload(dir: &std::path::Path) -> HookPayload {
        HookPayload { event: HookEvent::TurnStart, session_id: "s1".into(), timestamp: chrono::Utc::now(), status: HookStatus::Running, working_dir: dir.into(), config_dir: dir.join("config"), session_dir: dir.join("session"), error: None, reason: None }
    }
    ...
    fn runs_in_registration_order_and_continues() {
        ...
        let hooks = HookConfiguration { turn_start: vec!["one".into(), "two".into(), "three".into()], turn_stop: vec![] };
        let result = HookRunner::new(Duration::from_secs(2), 1024).run(&hooks, HookEvent::TurnStart, &payload(dir.path()), &DiagLogger::noop());
        ...
        assert!(output.find("one:turn-start:s1").unwrap() < output.find("two:turn-start:s1").unwrap());
    ...
    fn direct_spawn_and_timeout_are_safe() {
        ...
        fs::set_permissions(&path, fs::Permissions::from_mode(0o755)).unwrap();
        let hooks = HookConfiguration { turn_start: vec![path.file_name().unwrap().to_string_lossy().into()], turn_stop: vec![] };
        let result = HookRunner::new(Duration::from_millis(30), 1).run(&hooks, HookEvent::TurnStart, &payload(dir.path()), &DiagLogger::noop());
