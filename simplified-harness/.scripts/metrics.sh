...
section "Binary Size"
if [ -f target/release/orchid-simplified ]; then
  bytes=$(wc -c < target/release/orchid-simplified | tr -d ' ')
  ok "target/release/orchid-simplified → $(ls -lh target/release/orchid-simplified | awk '{print $5}') (${bytes} bytes)"
else
