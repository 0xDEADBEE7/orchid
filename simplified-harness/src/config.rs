...
impl Settings {
    pub fn prompt(&self) -> io::Result<String> {
        let name = self.policy.prompt.as_deref().unwrap_or("default");
...
impl Settings {
    ...
    fn codex_credential(&self, name: &str) -> io::Result<Credential> {
        let path = self.root.join("auth/tokens").join(format!("{name}.json"));
        let value: Value =
            read_json(&path).map_err(|_| safe_config_error("Codex credential unavailable"))?;
        let access_token = value["access_token"]
            .as_str()
            .ok_or_else(|| safe_config_error("Codex credential unavailable"))?;
        let account_id = value["account_id"].as_str().unwrap_or_default();
        if access_token.is_empty() {
            return Err(safe_config_error("Codex credential unavailable"));
        }
        Ok(Credential::Codex {
...
pub fn init(root: &Path) -> io::Result<()> {
    for name in ["policies", "connections", "auth", "prompts"] {
        fs::create_dir_all(root.join(name))?;
    }
    write_default(
        &root.join("config.json"),
        br#"{"policy":"default","log_level":"debug"}
"#,
    )?;
    write_default(
        &root.join("policies/default.json"),
        br#"{"prompt":"default","max_tokens":4096,"tools":[]}
"#,
    )?;
    write_default(
...
fn auth_login(settings: &Settings, name: Option<&String>) -> io::Result<String> {
    ...
    fs::write(settings.root.join("auth").join(name), body)?;
