...
pub fn run(
    ...
    for _ in 0..100 {
        budget(session, prompt, settings.policy.max_tokens())?;
...
fn budget(session: &mut Session, pending: &str, limit: u32) -> io::Result<()> {
    let estimate = pending.chars().count().div_ceil(4) as u32;
    if session.state.token_estimate.saturating_add(estimate) > limit {
        ...
        session.state.token_estimate += estimate;
        Ok(())
    }
}
fn record_usage(session: &mut Session, usage: Option<Usage>) {
    if let Some(tokens) = usage {
        session.state.token_estimate += tokens.input + tokens.output;
