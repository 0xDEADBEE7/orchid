...
- **Phase 3 — provider boundary assessment/extraction:** after `6326bb4`, inspected `src/client/codex.rs` and OpenAI/Anthropic provider modules. The safe cohesive boundary was Codex OAuth token persistence, validation, and refresh: moved unchanged behavior to `src/client/codex_auth.rs`, retained the existing `codex` exports and client call sites, and added characterization tests for token round-trip/validation plus missing and malformed files. Focused Codex/auth and provider tests pass; `make check` and `make metrics` pass. Fresh metrics report 6,937 production Rust lines, 3,247 test Rust lines, Codex auth 88 LOC/CC 14, Codex client 503 LOC/CC 52, and six red files. No shared transport or wire-mapping extraction is recommended yet: provider transport and mappings are materially provider-specific; assess session boundaries next.
