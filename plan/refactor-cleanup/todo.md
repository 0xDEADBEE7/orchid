...
- **Phase 1 — CLI parser:** active; help dispatch/validation extraction from `src/cli/parser.rs` is complete in commit `NEXT`; focused CLI tests and `make check` pass. `make metrics` reports parser `parse` CC 27 (down from 30 at 199 LOC; parser now 192 LOC), with six red files remaining. Command tokenization, lifecycle parsing, and other command-family dispatch were not changed.
