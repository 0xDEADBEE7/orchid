...
- **Phase 2 — execution loop:** token warning eligibility extraction completed after `3c5ed51`. Private `should_warn_for_budget` owns only repeated threshold/interval eligibility; warning emission, budget termination, token estimate metadata, lifecycle finalization, and public APIs remain unchanged. Focused loop tests, `make check`, and `make metrics` pass. Fresh metrics report `run_loop` CC 14, `src/loop/run.rs` 485 LOC, and six red files remain.
- **Phase 3 — session boundary:** after `5b77e02`, extracted session persistence/update logic into `src/session/persistence.rs`; `SessionStore` retains the public model and path APIs while atomic JSON writes and state/metadata merge behavior remain unchanged. Existing focused session/update characterization tests pass, including merge preservation, failed-write non-mutation, and on-disk state/metadata contracts. `session/mod.rs` is now 215 LOC; `update` is CC 1 and persistence `update` remains CC 17. `make check` and `make metrics` pass; six red files remain.
- **Next delegation — Phase 3 provider boundary:** split `src/client/codex.rs` further only where responsibility boundaries are clear, preserving public provider/auth APIs and behavior. The prior auth extraction is `5b77e02`; current worktree has an unrelated uncommitted model-list change in `src/client/codex.rs` that must be preserved and not included in the task commit. Await agent summary and review its commit before marking progress.
...
- [x] Reduce `run_loop` below CC 15. Private `should_warn_for_budget` extracts only repeated token warning eligibility; fresh metrics report CC 14. `run.rs` is 485 LOC and six red files remain.
- [x] Re-run `make check` and `make metrics` (formatting passes; six metrics red files remain).
...
- [x] Complete [audit.md](audit.md).
- [x] Remove confirmed dead implementation (none confirmed by the audit).
- [x] Remove/update obsolete tests and references (all reviewed rejection tests are contractual; none removed).
- [x] Reconcile contradictory architecture docs.
- [x] Archive completed plans where appropriate (deferred: retained as useful history; no archive convention).
