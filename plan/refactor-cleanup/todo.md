...
- **Phase 1 — CLI parser:** active; config command dispatch/validation extraction from `src/cli/parser.rs` into the owned `src/cli/config.rs` is complete in commit `12b9669`. The `config` and legacy top-level `validate` paths now share the config-owned parser while preserving command representation and exact errors. Focused config parser tests and `make check` pass. `make metrics` reports parser `parse` at CC 28 and 192 LOC (the prior recorded result was CC 27); six red files remain. Tokenization, global option extraction, help, lifecycle parsing, and unrelated command families were not changed.
