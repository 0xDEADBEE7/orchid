...
- **Phase 1 follow-up — await validation/parsing extraction:** complete in this run; moved cohesive `await` ID validation, timeout/interval parsing, defaults, and unknown-flag validation from `src/cli/parser.rs` to `src/cli/await_command.rs`, preserving exact errors and global flag merging. Focused await and CLI parser tests, `make test`, `make check`, and `make metrics` pass; parser `parse` CC is 32, parser is 239 LOC, and six red files remain.
