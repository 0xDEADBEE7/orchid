...
- **Phase 2 — execution loop:** terminal finalization centralization completed in this run after `a30b7c6`. `finish_loop` and `terminate_for_budget` consolidate successful and budget terminal paths without changing `RunGuard` ownership, lifecycle statuses, persistence, hooks, or contracts. Focused loop/lifecycle tests, `make check`, and `make metrics` pass. `run_loop` is CC 25, `src/loop/run.rs` is 455 LOC, and six red files remain.
