...
- **Phase 1 follow-up — list validation/parsing extraction:** complete in this run; moved list resource validation and command construction from `src/cli/parser.rs` to `src/cli/list.rs`, preserving the accepted resources, exact unknown-resource error, command representation, and global flag merging. Added characterization coverage for all accepted resources and the stable rejection message. `make test`, `make check`, and `make metrics` pass; parser is 228 LOC and `parse` is CC 31; six red files remain.
