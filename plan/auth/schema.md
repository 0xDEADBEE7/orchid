...
Add an `auth` resource directory containing one JSON object per safe resource name. A profile has a known `type` and a `value` reference. Resource names must use the same safe-name rules as other configuration resources.
