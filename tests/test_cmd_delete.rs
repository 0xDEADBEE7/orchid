...
fn test_list_uses_selected_config_sessions() {
    ...
    let selected_meta = selected_store
        .create(Some("selected".to_string()), None, None)
        .unwrap();
    other_store
        .create(Some("other".to_string()), None, None)
        .unwrap();
...
// What it tests: Verifying that deleting a non-existent session (fake 32-char ID)
...
// What it tests: Verifying that deleting an existing session archives it:
// the session directory is removed, and the session appears in .archive.
