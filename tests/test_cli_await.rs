...
fn await_binary_reports_timeout_with_exit_code_two() {
    ...
    assert_eq!(
        value,
        serde_json::json!({"completed": [], "timed_out": true})
    );
...
fn await_binary_reports_missing_state_as_json_error_without_mutation() {
    ...
    assert!(error["message"]
        .as_str()
        .unwrap()
        .contains("session state is missing"));
    assert_eq!(
        std::fs::read(store.metadata_path(&session.id)).unwrap(),
        metadata_before
    );
...
fn await_binary_reports_malformed_state_as_json_error_without_mutation() {
    ...
    assert!(error["message"]
        .as_str()
        .unwrap()
        .contains("invalid state JSON"));
    assert_eq!(
        std::fs::read(store.state_path(&session.id)).unwrap(),
        malformed
    );
    assert_eq!(
        std::fs::read(store.metadata_path(&session.id)).unwrap(),
