...
fn connection(interface: &str, credential: Option<Credential>) -> ResolvedConnection {
    ResolvedConnection {
        connection: Connection {
            interface: interface.into(), base_url: "http://localhost".into(),
            model: "test-model".into(), api_key: None, auth: None,
            params: Default::default(), headers: Default::default(),
        }, credential, params: Default::default(), headers: Default::default(),
    }
}

#[test]
fn factory_selects_codex_for_interface_and_oauth() {
    assert!(client_for(connection("codex", Some(Credential::Codex {
        access_token: "token".into(), account_id: "account".into()
    }))).is_ok());
...
fn codex_sse_maps_extracted_json_payload() {
    assert_eq!(orchid::client::openai_codex::parse_sse(r#"{"type":"response.completed"}"#), vec![ClientEvent::Done]);
}
